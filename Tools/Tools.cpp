...
auto run_timer(const std::string& name, int runs, std::size_t n_scanned, F f) { 
    ...
    std::cout << "\n median: " << median << " ms spread: " << times.front() << " - " << 
        times.back() << " ms\n" << " cost per element scanned: " << std::setprecision(2) << 
        (median * 1e6 / n_scanned) << " ns\n\n"; 
    return last; 
}
